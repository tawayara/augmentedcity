...
public class CustomActivity extends AndARActivity {
	 ...
	 * This exception is rather severe and can not be recovered from. Inform the
	 * user and shut down the application.
	 */
	@Override
	...
	public boolean onCreateOptionsMenu(Menu menu) {
		menu.add(0, MENU_SCREENSHOT, 0,
				getResources().getText(R.string.takescreenshot));
		return true;
	}
	...
	public boolean onOptionsItemSelected(MenuItem item) {
		/*
		 * if(item.getItemId()==1) { artoolkit.unregisterARObject(someObject); }
		 * else if(item.getItemId()==0) { try { someObject = new CustomObject
		 * ("test", "patt.hiro", 80.0, new double[]{0,0});
		 * artoolkit.registerARObject(someObject); } catch (AndARException e) {
		 * e.printStackTrace(); } }
		 */
