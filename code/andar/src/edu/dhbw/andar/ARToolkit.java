...
public class ARToolkit {
	...
	/**
	 * Registers an object to the ARToolkit. This means that the toolkit will try to determine the
	 * pose of the object. If the object is visible, it's draw method will be invoked. The
	 * corresponding translation matrix will be applied inside opengl before doing so.
	 * 
	 * Register objects with the same pattern twice will not work. Because the arloadpatt would
	 * create different IDs for the same pattern, and the detecting function could return only the
	 * first detected id. We need to store patt load id's in an hash -> loadpatt as a native
	 * function returning the ID -> pass this id to the object registering function.
	 * 
	 * @param arobject
	...
	public synchronized void registerARObject(ARObject arobject) throws AndARException {
		...
		try {
			...
			IO.transferFileToPrivateFS(baseFolder, arobject.getPatternName(), res);
			...
			addObject(nextObjectID, arobject, patternFile, arobject.getMarkerWidth(),
					arobject.getCenter());
			nextObjectID++;
		} catch (IOException e) {
			e.printStackTrace();
	...
	/**
	 * Register a object to the native library. From now on the detection function will determine if
	 * the given object is visible on a marker, and set the transformation matrix accordingly.
	 * 
	 * @param id
	 ...
	 */
	private native void addObject(int id, ARObject obj, String patternName, double markerWidth,
			double[] markerCenter);
	 ...
	 *            the image
	 * @param matrix
	 *            the transformation matrix for each marker, will be locked right before the trans
	...
	/**
	 * Inverse a three by four matrix. Matrix has to be 3 by 4! (but is actually a 4 by 4 homogene
	 * matrix.)
	 * 
	 * @param mat1
	 *            contains the matrix to be inversed.
	 * @param mat2
	 *            will contain the resulting matrix.
	 * @return success?
	 */
	public native static int arUtilMatInv(double[] mat1, double[] mat2);

	/**
	 * Multiply one (three by four) matrix by another. Matrix has to be 3 by 4! (but is actually a 4
	 * by 4 homogene matrix.)
	 * 
	 * @param multiplier
	 * @param multiplicand
	 * @param result
	 *            contains the result, after the method returns.
	 * @return
	 */
	public native static int arUtilMatMul(double[] multiplier, double[] multiplicand,
			double[] result);
	...
	private void initialize() {
		...
		if (screenWidth > 0 && screenHeight > 0 && imageWidth > 0 && imageHeight > 0) {
			...
			artoolkit_init(baseFolder + File.separator + calibFileName, imageWidth, imageHeight,
